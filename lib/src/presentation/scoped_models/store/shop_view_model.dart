...
import 'package:bulk_buyers/core/services/dynamic_links_services.dart';
...
import 'package:bulk_buyers/src/data/datasource/remote/model/store_api_provider.dart';
...
class ShopViewModel extends BaseModel {
  ...
  final DynamicLinksService _dynamicLinksService = di<DynamicLinksService>();
  ...
  var repo = new RepositoryImpl();

  Future populateShop() async {
    //  productsBox = Hive.box('store_products');
  ...
  Future rePopulateShop() async {
    setState(ViewState.Busy);
    // productsBox = await Hive.box('store_products');
    await _dynamicLinksService.handleDynamicLinks();
    await this.refresh();
    var res = repo.refreshProducts();
    notifyListeners();
    print(res);
  }
  ...
  Future fecthStoreProducts() async {
    ...
    //   print(" this is the box length: ${productsBox.length}");

    _productData = List<ProductsModels>.generate(
        _productData.length,
    ...
    // print("this is the debugged Product Box Lenght: ${productsBox.lenght}");
  ...
  * */
  updateWishlist(int id, bool state) async {
    print("this is id: $id this is state: $state");
    var reponse = await db.addToWishList(id, state);

    print("this is the response $reponse");
    this.fecthStoreProducts();
    this.fetchWishList();
    notifyListeners();
  }

  updateWishlistHive(int index) {
...
//    );
  ...
  Future fetchWishList() async {
    setState(ViewState.Busy);
//    var filteredUsers =
//        productsBox.values.where((product) => product.wishlist == true);
//    print(filteredUsers);

//    var details = ShopHiveModle.fromMap(filteredUsers) as ShopHiveModle;
//    print(details.productname);
    //  var filteredUsers = productsBox.values.where((user) => user.name.startsWith('s'));

    _wishlist = await db.getWishLists();
    print(_wishlist);
    var prod = await db.getAllProducts();
    print("this is the wishlist $prod");
    ...
    _wishlist = List<ProductsModels>.generate(
        _wishlist.length,
        (index) => ProductsModels(
